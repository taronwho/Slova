...
const firstCard = await p.evaluate(() => {
  ...
  const btn = card?.querySelector('.btn-primary')?.getBoundingClientRect()
  return { top: Math.round(r?.top ?? -1), playBottom: Math.round(btn?.bottom ?? -1), vh: document.documentElement.clientHeight }
})
...
for (const mode of ['Voština','Věž']) {
  await p.goto('http://localhost:4173/', { waitUntil:'networkidle' })
  await p.locator('.mode-card', { hasText: mode }).getByText('Hrát').click()
  await p.waitForSelector('.tut-card', { timeout: 20000 })
...
await dp.locator('.mode-card', { hasText:'Věž' }).getByText('Hrát').click()
